...
// ============================================================================
// 多线程并行版本：将随机重启 trials 分配到多个线程
// 每个线程独立运行一部分 trials，最后汇总取最优解
// ============================================================================

Solution::TrialResult Solution::run_trials_batch(Graph &graph,
                                                   int start_trial, int num_trials,
                                                   int total_nodes, int max_degree,
                                                   int num_nets,
                                                   int min_part_size, int max_part_size) {
    // 每个线程创建独立的 Solution 实例，无共享数据
    Solution local;

    local.total_nodes_ = total_nodes;
    local.max_degree_ = max_degree;
    local.gain_offset_ = max_degree;
    local.min_part_size_ = min_part_size;
    local.max_part_size_ = max_part_size;

    // 分配数组 (1-indexed)
    local.part_.assign(total_nodes + 1, 0);
    local.gain_.assign(total_nodes + 1, 0);
    local.locked_.assign(total_nodes + 1, false);
    local.bucket_next_.assign(total_nodes + 1, -1);
    local.bucket_prev_.assign(total_nodes + 1, -1);

    local.net_count_X_.assign(num_nets, 0);
    local.net_count_Y_.assign(num_nets, 0);

    // 初始化桶头数组
    int bucket_size = 2 * max_degree + 1;
    for (int s = 0; s < 2; s++) {
        local.bucket_heads_[s].assign(bucket_size, -1);
    }

    TrialResult best_local;
    best_local.cut = INT_MAX;
    best_local.part.assign(total_nodes + 1, 0);

    // 运行分配给本线程的 trials
    for (int t = 0; t < num_trials; t++) {
        int trial_id = start_trial + t;
        mt19937 rng(trial_id * 12345 + 42); // 确定性种子

        // 随机初始划分
        local.init_partition(graph, rng);

        // 初始化超网分布计数
        fill(local.net_count_X_.begin(), local.net_count_X_.end(), 0);
        fill(local.net_count_Y_.begin(), local.net_count_Y_.end(), 0);
        for (Node *node : graph.get_nodes()) {
            int nid = node->get_index();
            for (Net *net : node->get_nets()) {
                int net_id = net->get_index();
                if (local.part_[nid] == 0)
                    local.net_count_X_[net_id]++;
                else
                    local.net_count_Y_[net_id]++;
            }
        }

        // 多 Pass 迭代
        while (true) {
            int prev_cut = local.compute_cut_size(num_nets);
            local.fm_pass(graph);
            ...
            int curr_cut = local.compute_cut_size(num_nets);
            int improvement = prev_cut - curr_cut;
            if (improvement <= 0) break;
        }

        int final_cut = local.compute_cut_size(num_nets);
        if (final_cut < best_local.cut) {
            best_local.cut = final_cut;
            best_local.part = local.part_;
        }
    }
...
void Solution::parallel_partition(Graph &graph, set<int> &X, set<int> &Y, int num_threads) {
    int total_nodes = graph.get_node_num();
    int num_nets = graph.get_net_num();

    // 计算最大度数
    int max_degree = 0;
    for (Node *node : graph.get_nodes()) {
        max_degree = max(max_degree, (int)node->get_nets().size());
    }
    if (max_degree < 1) max_degree = 1;

    // 平衡约束
    int min_part_size = (int)ceil(0.48 * total_nodes);
    int max_part_size = (int)floor(0.52 * total_nodes);

    // 确定 trial 总数
    int total_trials = (total_nodes < 50000) ? 5 : 2;

    // 确保线程数不超过 trial 数
    int actual_threads = min(num_threads, total_trials);

    // 计算每个线程分配的 trial 数
    vector<int> trials_per_thread(actual_threads, total_trials / actual_threads);
    for (int i = 0; i < total_trials % actual_threads; i++) {
        trials_per_thread[i]++;
    }

    // 启动线程
    vector<thread> threads;
    vector<TrialResult> results(actual_threads);

    int trial_start = 0;
    for (int i = 0; i < actual_threads; i++) {
        int start = trial_start;
        int count = trials_per_thread[i];
        trial_start += count;

        threads.emplace_back([&graph, start, count, total_nodes, max_degree, num_nets,
                              min_part_size, max_part_size, &results, i]() {
            results[i] = run_trials_batch(graph, start, count, total_nodes,
                                           max_degree, num_nets,
    ...
    // 汇总结果，选取最优
    int best_cut = INT_MAX;
    vector<int> best_part(total_nodes + 1, 0);
    ...
    cout << "Parallel FM (threads=" << actual_threads << ") best cut = " << best_cut << endl;
}
