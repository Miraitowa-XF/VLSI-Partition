...
public:
    ...
    // ========== 多线程并行划分 ==========
    // 将随机重启 trials 分配到 num_threads 个线程并行执行
    static void parallel_partition(Graph &graph, set<int> &X, set<int> &Y, int num_threads);
...
private:
    ...
    // 运行一批 trials (每个线程调用此函数)
    static TrialResult run_trials_batch(Graph &graph, int start_trial, int num_trials,
                                         int total_nodes, int max_degree, int num_nets,
