...
// 读取 benchmark 网表文件：解析超网边并构建图结构（节点与网的双向关联）
...
// 增益桶插入：将节点按增益值插入对应分区的桶链表头部，更新桶内最大增益指针
...
// 增益桶移除：从分区桶链表中摘除节点，并修复 max_gain_in_bucket 指针
...
// 从增益桶中选择最佳移动节点：遍历各分区桶，选出满足平衡约束且增益最大的 (节点, 源分区, 目标分区)
...
// 随机初始分区：将节点随机排列后均匀分配给各 FPGA，确保每个 FPGA 大小接近平衡
...
// 计算各节点的初始移动增益：统计 net_count 后对每个节点评估移动到各 FPGA 的 cut 增益
...
// 构建增益桶：清空并重建各分区的桶链表，将所有未锁定节点插入桶中
...
// 移动后增量更新增益：遍历受影响网的所有邻居节点，先减去旧贡献再更新 net_count 后加上新贡献，并重新插入桶
...
// 计算当前分区的 cut size：统计每个网跨越的 FPGA 数量，按 (span - 1) 累加
...
// 一次 FM 优化轮次：初始化增益与桶，迭代选取最佳移动并回溯到最佳中间状态，输出移动日志
...
// 主分区算法入口：初始化数据结构 → 多 trial 随机初始分区 + FM 多轮迭代优化 → 输出最优分区结果
void Solution::my_partition_algorithm(Graph &graph, int K, double r, vector<int> &part_result) {
    ...
    // [FIX-ibm02-K3] 当处于严格均衡(r>=1/K 等价)时，收紧 max 到 ceil(N/K)，
    // 避免 N mod K >= 2 时余数全部堆进同一块导致 max-min>1。
    // 松弛平衡(r<1/K)时 min_part_size_ < floor(N/K)，保持原 max 不动。
    {
        int ceil_nk = (total_nodes_ + K_ - 1) / K_;          // ceil(N/K)
        int floor_nk = total_nodes_ / K_;                    // floor(N/K)
        if (min_part_size_ >= floor_nk) {                    // 等价于 r*|n >= 1 的严格均衡区间
            if (max_part_size_ > ceil_nk) max_part_size_ = ceil_nk;
        }
    }
