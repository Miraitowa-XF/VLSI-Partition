...
// 单个线程执行的试验：包含完整的初始化和多次FM pass
void Solution::run_single_trial(Graph &graph, int trial_id, unsigned int seed,
                                vector<int> &local_part, int &local_cut, int &local_violations,
                                atomic<bool> &should_stop) {
    // 每个线程有自己独立的随机数生成器
    mt19937 rng(seed);
    
    // 分配线程局部数据结构（复制主数据结构的状态）
    int alloc_size = total_nodes_ + 2;
    vector<int> part(alloc_size, -1);
    vector<vector<int>> gain(alloc_size, vector<int>(K_, 0));
    vector<int> max_gain(alloc_size, -1e9);
    vector<int> best_dest(alloc_size, -1);
    vector<bool> locked(alloc_size, false);
    vector<vector<int>> net_count(graph.get_net_num(), vector<int>(K_, 0));
    vector<int> current_size(K_, 0);
    vector<vector<int>> topo_delta(alloc_size, vector<int>(K_, 0));
    vector<int> topo_delta_version(alloc_size, -1);
    int topo_epoch = 0;
    
    // 桶结构
    vector<vector<int>> bucket_heads(K_, vector<int>(2 * max_degree_ + 1, -1));
    vector<int> bucket_next(alloc_size, -1);
    vector<int> bucket_prev(alloc_size, -1);
    vector<int> max_gain_in_bucket(K_, -max_degree_ - 1);
    vector<bool> in_bucket(alloc_size, false);
    vector<int> neighbor_seen(alloc_size, 0);
    int neighbor_epoch = 0;
    
    // 初始化划分（复制主线程的Cddt_mask约束）
    fill(current_size.begin(), current_size.end(), 0);
    
    // 将所有节点放在拥有最多固定节点的 FPGA 上
    int best_fpga = 0, best_cnt = -1;
    for (int f = 0; f < K_; f++) {
        int c = 0;
        for (Node *node : graph.get_nodes()) {
    ...
    for (Node *node : graph.get_nodes()) {
        int v = node->get_index();
        if (is_fixed[v]) { part[v] = fixed_fpga[v]; current_size[fixed_fpga[v]]++; }
        else { part[v] = best_fpga; current_size[best_fpga]++; }
    }
    
    // 初始化 net_count
    for (Node *node : graph.get_nodes()) {
        int v = node->get_index();
        for (Net *net : node->get_nets()) net_count[net->get_index()][part[v]]++;
    }
    ...
    for (int iter = 0; iter < K_ * K_; iter++) {
        int over_fpga = -1, over_size = -1;
        for (int f = 0; f < K_; f++) {
            if (current_size[f] > max_size_[f] && current_size[f] > over_size) {
                over_size = current_size[f]; over_fpga = f;
            }
        }
        if (over_fpga == -1) break;
        ...
        for (Node *node : graph.get_nodes()) {
            int v = node->get_index();
            if (is_fixed[v] || part[v] != over_fpga) continue;
            for (int t = 0; t < K_; t++) {
                if (t == over_fpga) continue;
                if (current_size[t] >= max_size_[t]) continue;
                bool ok = true;
                for (Net *net : node->get_nets()) {
                    int nid = net->get_index();
                    for (int g = 0; g < K_; g++) {
                        if (g == t) continue;
                        int cnt = net_count[nid][g];
                        if (g == over_fpga) cnt--;
                        if (cnt > 0 && !topo_adj[g][t]) { ok = false; break; }
                    }
                    if (!ok) break;
        ...
        if (move_node == -1) break;
        int old_f = part[move_node];
        part[move_node] = move_target;
        current_size[old_f]--; current_size[move_target]++;
        Node *mn = graph.get_node(move_node);
        if (mn) for (Net *net : mn->get_nets()) {
            net_count[net->get_index()][old_f]--;
            net_count[net->get_index()][move_target]++;
        }
    }
    
    // 计算初始增益
    for (int i = 0; i < (int)net_count.size(); ++i) fill(net_count[i].begin(), net_count[i].end(), 0);
    for (Node *node : graph.get_nodes()) {
        int nid = node->get_index(); int P = part[nid];
        for (Net *net : node->get_nets()) net_count[net->get_index()][P]++;
    }
    for (Node *node : graph.get_nodes()) {
        int nid = node->get_index(); int P = part[nid];
        max_gain[nid] = -1e9;
        for (int X = 0; X < K_; ++X) {
            if (X == P) { gain[nid][X] = 0; topo_delta[nid][X] = 0; continue; }
            if (!Cddt_mask[nid][X]) { gain[nid][X] = -1e9; topo_delta[nid][X] = 1e9; continue; }
            int g = 0;
            int td = 0;
            for (Net *net : node->get_nets()) {
                int net_id = net->get_index();
                if (net_count[net_id][P] == 1) g++;
                if (net_count[net_id][X] == 0) g--;
                int old_v = 0, new_v = 0;
                for (int f = 0; f < K_; f++) {
                    if (f != P && !topo_adj[P][f]) old_v += net_count[net_id][f];
                }
                for (int f = 0; f < K_; f++) {
                    if (f == X) continue;
                    int cnt = net_count[net_id][f];
                    if (f == P) cnt--;
                    if (!topo_adj[X][f]) new_v += cnt;
                }
                td += new_v - old_v;
            ...
            gain[nid][X] = g;
            topo_delta[nid][X] = td;
            if (g > max_gain[nid]) { max_gain[nid] = g; best_dest[nid] = X; }
        }
        topo_delta_version[nid] = topo_epoch;
    }
    
    // 辅助lambda：计算topo delta
    auto compute_topo_delta_local = [&](int node, int target_block) -> int {
        ...
        for (Net *net : n->get_nets()) {
            ...
            for (int f = 0; f < K_; f++) {
                if (f == old_block) continue;
                if (!topo_adj[old_block][f]) old_v += net_count[nid][f];
            }
            for (int f = 0; f < K_; f++) {
                if (f == target_block) continue;
                int cnt = net_count[nid][f];
                if (f == old_block) cnt--;
                if (!topo_adj[target_block][f]) new_v += cnt;
            }
            delta += new_v - old_v;
    ...
    // 辅助lambda：桶插入
    auto bucket_insert_local = [&](int node_id) {
        int P = part[node_id];
        int idx = max_gain[node_id] + gain_offset_;
        bucket_prev[node_id] = -1;
        bucket_next[node_id] = bucket_heads[P][idx];
        if (bucket_heads[P][idx] != -1) bucket_prev[bucket_heads[P][idx]] = node_id;
        bucket_heads[P][idx] = node_id;
        if (max_gain[node_id] > max_gain_in_bucket[P]) max_gain_in_bucket[P] = max_gain[node_id];
        in_bucket[node_id] = true;
    };
    
    // 辅助lambda：桶移除
    auto bucket_remove_local = [&](int node_id) {
        if (!in_bucket[node_id]) return;
        in_bucket[node_id] = false;
        int P = part[node_id];
        int idx = max_gain[node_id] + gain_offset_;
        if (bucket_prev[node_id] != -1) bucket_next[bucket_prev[node_id]] = bucket_next[node_id];
        else {
            bucket_heads[P][idx] = bucket_next[node_id];
            if (bucket_heads[P][idx] == -1 && max_gain[node_id] == max_gain_in_bucket[P]) {
                while (max_gain_in_bucket[P] >= -max_degree_ && bucket_heads[P][max_gain_in_bucket[P] + gain_offset_] == -1)
                    max_gain_in_bucket[P]--;
            }
        }
        if (bucket_next[node_id] != -1) bucket_prev[bucket_next[node_id]] = bucket_prev[node_id];
        bucket_next[node_id] = -1; bucket_prev[node_id] = -1;
    };
    
    // 辅助lambda：获取最佳节点
    auto bucket_get_best_node_local = [&](int &from_side, int &to_side) -> int {
        int best_node = -1;
        int max_valid_gain = -1e9;
        int best_violation_delta = 1e9;
        int nodes_checked = 0;
        const int MAX_NODES_CHECK = 5000;
        for (int k = 0; k < K_; ++k) {
            if (current_size[k] - 1 < min_size_[k]) continue;
            for (int g = max_gain_in_bucket[k]; g >= -max_degree_; --g) {
                if (g < max_valid_gain - 1) break;
                if (nodes_checked >= MAX_NODES_CHECK) return best_node;
                int curr = bucket_heads[k][g + gain_offset_];
                while (curr != -1) {
                    if (++nodes_checked > MAX_NODES_CHECK) return best_node;
                    if (is_fixed[curr]) { curr = bucket_next[curr]; continue; }
                    int P = part[curr];
                    if (topo_delta_version[curr] != topo_epoch) {
                        for (int X = 0; X < K_; ++X) {
                            if (X == P) { topo_delta[curr][X] = 0; continue; }
                            if (!Cddt_mask[curr][X]) { topo_delta[curr][X] = 1e9; continue; }
                            topo_delta[curr][X] = compute_topo_delta_local(curr, X);
                        }
                        topo_delta_version[curr] = topo_epoch;
                    }
                    for (int X = 0; X < K_; ++X) {
                        if (X == k) continue;
                        if (!Cddt_mask[curr][X]) continue;
                        if (current_size[X] + 1 > max_size_[X]) continue;
                        int v_delta = topo_delta[curr][X];
                        if (v_delta > 0) continue;
                        int score = gain[curr][X];
                        if (v_delta < best_violation_delta ||
                            (v_delta == best_violation_delta && score > max_valid_gain)) {
                            best_violation_delta = v_delta;
                            max_valid_gain = score;
    ...
    // 辅助lambda：更新增益
    auto update_gains_after_move_local = [&](int moved_node, int from_side, int to_side) {
        neighbor_epoch++;
        const int NET_PIN_THRESHOLD = 500;
        ...
        for (Net *net : graph.get_node(moved_node)->get_nets()) {
            ...
            for (Node *cell : net->get_nodes()) {
                int cid = cell->get_index();
                if (locked[cid]) continue;
                if (neighbor_seen[cid] != neighbor_epoch) {
                    neighbor_seen[cid] = neighbor_epoch;
                    if (cid != moved_node) affected.push_back(cid);
                }
            }
        }
        for (int cid : affected) bucket_remove_local(cid);
        ...
        for (Net *net : graph.get_node(moved_node)->get_nets()) {
            ...
            if (net_size <= NET_PIN_THRESHOLD) {
                for (Node *cell : net->get_nodes()) {
                    ...
                    for (int X = 0; X < K_; ++X) {
                        if (X == P) continue;
                        if (!Cddt_mask[cid][X]) { gain[cid][X] = -1e9; continue; }
                        int old_contrib = (net_count[net_id][P] == 1 ? 1 : 0) - (net_count[net_id][X] == 0 ? 1 : 0);
                        gain[cid][X] -= old_contrib;
                    }
                }
            }
            
            net_count[net_id][from_side]--; net_count[net_id][to_side]++;
            ...
            if (net_size <= NET_PIN_THRESHOLD) {
                for (Node *cell : net->get_nodes()) {
                    ...
                    for (int X = 0; X < K_; ++X) {
                        if (X == P) continue;
                        if (!Cddt_mask[cid][X]) { gain[cid][X] = -1e9; continue; }
                        int new_contrib = (net_count[net_id][P] == 1 ? 1 : 0) - (net_count[net_id][X] == 0 ? 1 : 0);
                        gain[cid][X] += new_contrib;
                    }
                }
            }
        }
        ...
        for (int cid : affected) {
            int P = part[cid];
            max_gain[cid] = -1e9; bool has_valid = false;
            for (int X = 0; X < K_; ++X) {
                if (X == P) continue;
                if (!Cddt_mask[cid][X]) { gain[cid][X] = -1e9; continue; }
                has_valid = true;
                if (gain[cid][X] > max_gain[cid]) { max_gain[cid] = gain[cid][X]; best_dest[cid] = X; }
            }
            if (has_valid) bucket_insert_local(cid);
            topo_delta_version[cid] = -1;
        }
    };
    ...
    while (!should_stop.load()) {
        fill(locked.begin(), locked.end(), false);
        ...
        for (Node *node : graph.get_nodes()) {
            ...
            max_gain[nid] = -1e9;
            for (int X = 0; X < K_; ++X) {
                if (X == P) { gain[nid][X] = 0; continue; }
                if (!Cddt_mask[nid][X]) { gain[nid][X] = -1e9; continue; }
                int g = 0;
                for (Net *net : node->get_nets()) {
                    int net_id = net->get_index();
                    if (net_count[net_id][P] == 1) g++;
        ...
        // 构建桶
        for (int s = 0; s < K_; s++) {
            fill(bucket_heads[s].begin(), bucket_heads[s].end(), -1);
            max_gain_in_bucket[s] = -max_degree_ - 1;
        }
        fill(bucket_next.begin(), bucket_next.end(), -1);
        fill(bucket_prev.begin(), bucket_prev.end(), -1);
        fill(in_bucket.begin(), in_bucket.end(), false);
        for (Node *node : graph.get_nodes()) {
            int i = node->get_index();
            if (!locked[i] && max_gain[i] > -1e8) bucket_insert_local(i);
        }
        ...
        while (true) {
            int from_side = -1, to_side = -1;
            int chosen = bucket_get_best_node_local(from_side, to_side);
            if (chosen == -1) break;
            
            bucket_remove_local(chosen); part[chosen] = to_side; locked[chosen] = true;
            current_size[from_side]--; current_size[to_side]++;
            cumulative_gain += gain[chosen][to_side];
            cumulative_topo += topo_delta[chosen][to_side];
            moves_node.push_back(chosen); moves_from.push_back(from_side); moves_to.push_back(to_side);
            ...
            update_gains_after_move_local(chosen, from_side, to_side);
        }
        
        // 回溯
        for (int i = (int)moves_node.size() - 1; i >= best_step; i--) {
            part[moves_node[i]] = moves_from[i];
            current_size[moves_to[i]]--;
            current_size[moves_from[i]]++;
        }
        
        // 检查是否收敛
        if (best_topo >= 0 && best_gain <= 0) break;
        pass++;
    ...
    // 违规修复
    for (int repair_iter = 0; repair_iter < 20 && !should_stop.load(); repair_iter++) {
        int repaired = 0;
        for (Net *net : graph.get_nets()) {
            vector<Node *> nodes = net->get_nodes();
            if (nodes.size() < 2) continue;
            bool fp[16]; for (int f = 0; f < K_; f++) fp[f] = false;
            for (Node *n : nodes) { int p = part[n->get_index()]; if (p >= 0 && p < K_) fp[p] = true; }
            bool violated = false;
            for (int f1 = 0; f1 < K_ && !violated; f1++) if (fp[f1])
                for (int f2 = f1+1; f2 < K_; f2++) if (fp[f2] && !topo_adj[f1][f2]) { violated = true; break; }
            if (!violated) continue;
            ...
            for (int target = 0; target < K_; target++) {
                ...
                vector<int> saved_part = part;
                vector<int> saved_size = current_size;
                vector<vector<int>> saved_netcount = net_count;
                bool any_moved = false;
                ...
                for (Node *n : nodes) {
                    ...
                    if (!safe) continue;
                    part[nid] = target;
                    current_size[old]--; current_size[target]++;
                    for (Net *nt : n->get_nets()) {
                        net_count[nt->get_index()][old]--;
                        net_count[nt->get_index()][target]++;
                    }
                    any_moved = true;
                }
                if (!any_moved) {
                    part = saved_part; current_size = saved_size; net_count = saved_netcount;
                    continue;
                }
                
                // 计算违规数
                int viol = 0;
                for (Net *nt : graph.get_nets()) {
                    vector<Node *> ns = nt->get_nodes();
                    if (ns.size() < 2) continue;
                    int anchor = part[ns[0]->get_index()];
                    for (size_t j = 1; j < ns.size(); j++) {
                        int pj = part[ns[j]->get_index()];
                        if (pj != anchor && !topo_adj[anchor][pj]) viol++;
                    }
                }
                
                if (viol < best_viol_after) {
                    best_viol_after = viol; best_target = target;
                    best_state_part = part;
                    best_state_size = current_size;
                    best_state_netcount = net_count;
                    if (viol == 0) break;
                }
                part = saved_part; current_size = saved_size; net_count = saved_netcount;
            }
            ...
            if (best_target != -1) {
                part = best_state_part;
                current_size = best_state_size;
                net_count = best_state_netcount;
                repaired++;
            }
        }
        if (repaired == 0) break;
    ...
    for (Net *net : graph.get_nets()) {
        const vector<Node *>& nodes = net->get_nodes();
        if (nodes.size() < 2) continue;
        int anchor = part[nodes[0]->get_index()];
        for (size_t j = 1; j < nodes.size(); ++j) {
            if (part[nodes[j]->get_index()] != anchor) cut++;
        }
    }
    local_cut = (int)cut;
    ...
    for (Net *net : graph.get_nets()) {
        vector<Node *> nodes = net->get_nodes();
        if (nodes.size() < 2) continue;
        int anchor_p = part[nodes[0]->get_index()];
        for (size_t j = 1; j < nodes.size(); ++j) {
            int pj = part[nodes[j]->get_index()];
            if (pj != anchor_p && !topo_adj[anchor_p][pj]) violations++;
        }
    }
...
// 多线程并行FM划分主入口
void Solution::my_partition_algorithm_parallel(Graph &graph, int K, vector<int> &part_result, int num_threads) {
    if (K != K_) {
        cerr << "Warning: K parameter (" << K << ") != topology FPGA count (" << K_ << "). Using topology count." << endl;
    }
    ...
    // 确定试验数量（固定为8次，与原始单线程版本保持一致）
    int total_trials = 8;  // 固定试验数量，不随线程数变化，确保公平对比
    cout << "Using " << num_threads << " threads for " << total_trials << " parallel trials." << endl;
    ...
    for (int t = 0; t < num_threads; t++) {
        workers.emplace_back([&, t]() {
            ...
            int extra = total_trials % num_threads;
            int my_trials = trials_per_thread + (t < extra ? 1 : 0);
            
            for (int local_trial = 0; local_trial < my_trials && !should_stop.load(); local_trial++) {
                int global_trial_id = t * trials_per_thread + min(t, extra) + local_trial;
                unsigned int seed = global_trial_id * 12345 + 42;
                
                vector<int> local_part;
                int local_cut = INT_MAX;
                int local_violations = INT_MAX;
                
                run_single_trial(graph, global_trial_id, seed, local_part, local_cut, local_violations, should_stop);
                ...
                int completed = completed_trials.fetch_add(1) + 1;
                if (completed % max(1, total_trials / 10) == 0) {
                    cout << "Progress: " << completed << "/" << total_trials << " trials completed" << endl;
                }
    ...
    cout << "All " << total_trials << " trials completed." << endl;
    cout << "Best result: cut=" << best_cut << ", violations=" << best_violations << endl;
