...
int main(int argc, char **argv) {
    ...
    if(argc < 2) {
        cout << "Usage: ./main <netlist_file> [topology_file] [K] [num_threads]" << endl;
        cout << "Example: ./main ./dataset/case1 ./dataset/MSF1 8 4" << endl;
        cout << "Note: num_threads=0 means auto-detect (default: single-threaded)" << endl;
        exit(-1);
    }
    ...
    if (argc >= 5) {
        try {
            ...
            cerr << "Error: num_threads must be an integer, got: " << argv[4] << endl;
            exit(-1);
        }
    }
    ...
    if (num_threads > 0) {
        cout << "Using multi-threaded mode with " << num_threads << " threads" << endl;
    } else if (num_threads == 0) {
        cout << "Using single-threaded mode (use num_threads > 0 for parallel)" << endl;
    }
    ...
    if (num_threads > 0) {
        solution.my_partition_algorithm_parallel(graph, K, part_result, num_threads);
    } else {
