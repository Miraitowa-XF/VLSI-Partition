...
public:
    ...
    // 多线程版本的多路 FM 划分
    void my_partition_algorithm_parallel(Graph &graph, int K, vector<int> &part_result, int num_threads = 0);
...
private:
    ...
    // ===== 多线程相关 =====
    int num_threads_;                       // 线程数（0表示自动检测）
    mutex best_result_mutex_;              // 保护最优结果的全局互斥锁
