...
// ========== OpenMP 开关配置 ==========
// 定义 _OPENMP 以启用并行加速（编译时需同时加 -fopenmp 选项）
#define _OPENMP
// ====================================
...
int main(int argc, char **argv) {
    ...
    if(argc < 2) {
        ...
        cout << "Example: ./main Titan23Benchmarks/des90 FPGAGraph/MFS1 8 1" << endl;
        cout << "  num_threads: OpenMP 线程数，默认使用系统默认值" << endl;
        exit(-1);
    }
    ...
    int K = 4;
    int num_threads = -1; // -1 表示使用默认值
    
    // 解析参数：判断 argv[3] 是 K 还是线程数
    if (argc >= 4) {
        try {
            K = stoi(argv[3]);
        } catch (...) {
    ...
    // 如果提供了第 4 个参数 (argv[4])，则将其解析为线程数
    if (argc >= 5) {
        try {
            num_threads = stoi(argv[4]);
            if (num_threads < 1) {
                cerr << "Error: num_threads must be at least 1, got: " << num_threads << endl;
                exit(-1);
            }
            omp_set_num_threads(num_threads);
            cout << "OpenMP threads set to: " << num_threads << endl;
        } catch (...) {
