...
int Solution::strict_topology_min_conflicts(Graph &graph, int time_limit_seconds) {
    if (!topology_enabled_ || time_limit_seconds <= 0) return 0;
    using Clock = std::chrono::steady_clock;
    const auto deadline = Clock::now() + std::chrono::seconds(time_limit_seconds);

    struct PairEdge { int u; int v; int net_id; };
    std::vector<PairEdge> edges;
    edges.reserve(static_cast<size_t>(graph.get_net_num()));
    std::vector<std::vector<int>> incident(static_cast<size_t>(total_nodes_));

    // TopoPart Generated Benchmarks are two-pin graphs.  For rare hyperedges we add all
    // pin pairs up to a small cap, because a hyperedge is topology-valid only when all
    // used FPGA parts form a clique in the MFS topology graph.
    for (const Net *net : graph.get_nets()) {
        const std::vector<Node *> &pins = net->get_nodes();
        if (pins.size() < 2) continue;
        if (pins.size() == 2) {
            int u = pins[0]->get_index();
            int v = pins[1]->get_index();
            int eid = static_cast<int>(edges.size());
            edges.push_back({u, v, net->get_index()});
            incident[static_cast<size_t>(u)].push_back(eid);
            incident[static_cast<size_t>(v)].push_back(eid);
        } else if (pins.size() <= 32) {
            for (size_t i = 0; i < pins.size(); ++i) {
                for (size_t j = i + 1; j < pins.size(); ++j) {
                    int u = pins[i]->get_index();
                    int v = pins[j]->get_index();
            ...
            int u0 = pins[0]->get_index();
            for (size_t j = 1; j < pins.size(); ++j) {
                int v = pins[j]->get_index();
                int eid = static_cast<int>(edges.size());
                edges.push_back({u0, v, net->get_index()});
                incident[static_cast<size_t>(u0)].push_back(eid);
                incident[static_cast<size_t>(v)].push_back(eid);
            }
        }
    }

    auto is_bad_pair = [&](int eid) -> bool {
        const PairEdge &e = edges[static_cast<size_t>(eid)];
        int pu = part_[static_cast<size_t>(e.u)];
        int pv = part_[static_cast<size_t>(e.v)];
        if (pu < 0 || pv < 0 || pu >= K_ || pv >= K_) return false;
        return topo_allowed_[static_cast<size_t>(pu)][static_cast<size_t>(pv)] == 0;
    };

    std::vector<char> edge_bad(edges.size(), 0);
    std::vector<int> incident_bad(static_cast<size_t>(total_nodes_), 0);
    std::vector<int> bad_edges;
    bad_edges.reserve(edges.size() / 16 + 1024);

    auto rebuild_bad_state = [&]() -> int {
        std::fill(edge_bad.begin(), edge_bad.end(), 0);
        std::fill(incident_bad.begin(), incident_bad.end(), 0);
        bad_edges.clear();
        int fixed_fixed_bad = 0;
        for (int eid = 0; eid < static_cast<int>(edges.size()); ++eid) {
            bool bad = is_bad_pair(eid);
            if (!bad) continue;
            edge_bad[static_cast<size_t>(eid)] = 1;
            const PairEdge &e = edges[static_cast<size_t>(eid)];
            ++incident_bad[static_cast<size_t>(e.u)];
            ++incident_bad[static_cast<size_t>(e.v)];
            bad_edges.push_back(eid);
            if (is_fixed(e.u) && is_fixed(e.v)) ++fixed_fixed_bad;
        }
        if (fixed_fixed_bad > 0) {
            cout << "Strict repair lower bound: " << fixed_fixed_bad
                 << " violated edges are between two fixed nodes and cannot be repaired without relaxing fixed constraints." << endl;
        }
        return static_cast<int>(bad_edges.size());
    };

    int bad_count = rebuild_bad_state();
    if (bad_count == 0) return 0;

    std::mt19937 rng(20260123);
    int total_moves = 0;
    long long steps = 0;
    long long stale_picks = 0;
    int best_bad = bad_count;
    std::vector<int> best_part = part_;

    auto count_bad_if_color = [&](int u, int color) -> int {
        int cnt = 0;
        for (int eid : incident[static_cast<size_t>(u)]) {
            const PairEdge &e = edges[static_cast<size_t>(eid)];
            int v = (e.u == u) ? e.v : e.u;
            int pv = part_[static_cast<size_t>(v)];
            if (pv < 0 || pv >= K_) continue;
            if (!topo_allowed_[static_cast<size_t>(color)][static_cast<size_t>(pv)]) ++cnt;
        }
        return cnt;
    };

    auto cut_delta_pair_if_color = [&](int u, int color) -> int {
        int from = part_[static_cast<size_t>(u)];
        if (from == color) return 0;
        int delta = 0;
        for (int eid : incident[static_cast<size_t>(u)]) {
            const PairEdge &e = edges[static_cast<size_t>(eid)];
            int v = (e.u == u) ? e.v : e.u;
            int pv = part_[static_cast<size_t>(v)];
            if (pv < 0 || pv >= K_) continue;
            int before = (from == pv) ? 0 : 1;
            int after = (color == pv) ? 0 : 1;
            delta += after - before;
        }
        return delta;
    };

    auto choose_color = [&](int u, bool force_change) -> int {
        int from = part_[static_cast<size_t>(u)];
        int current_bad = incident_bad[static_cast<size_t>(u)];
        int best_color = from;
        int best_conflicts = current_bad;
        int best_cut_delta = 0;
        std::vector<int> ties;
        ties.reserve(static_cast<size_t>(K_));

        for (int c = 0; c < K_; ++c) {
            if (c == from && force_change) continue;
            if (c != from) {
                if (current_size_[static_cast<size_t>(from)] <= min_part_size_) continue;
                if (current_size_[static_cast<size_t>(c)] >= max_part_size_) continue;
            }
            int conflicts = count_bad_if_color(u, c);
            int cut_delta = cut_delta_pair_if_color(u, c);
            bool better = false;
            if (conflicts < best_conflicts) better = true;
            else if (conflicts == best_conflicts && cut_delta < best_cut_delta) better = true;
            else if (conflicts == best_conflicts && cut_delta == best_cut_delta &&
                     c != from && topo_degree_[static_cast<size_t>(c)] > topo_degree_[static_cast<size_t>(best_color)]) better = true;
        ...
        if (!ties.empty() && best_conflicts <= current_bad) {
            std::uniform_int_distribution<int> pick(0, static_cast<int>(ties.size()) - 1);
            best_color = ties[static_cast<size_t>(pick(rng))];
        }
        return best_color;
    };

    auto apply_move = [&](int u, int to) {
        int from = part_[static_cast<size_t>(u)];
        if (from == to) return;
        // Remove old bad contributions for incident pair edges.
        for (int eid : incident[static_cast<size_t>(u)]) {
            if (!edge_bad[static_cast<size_t>(eid)]) continue;
            const PairEdge &e = edges[static_cast<size_t>(eid)];
            --incident_bad[static_cast<size_t>(e.u)];
            --incident_bad[static_cast<size_t>(e.v)];
            edge_bad[static_cast<size_t>(eid)] = 0;
            --bad_count;
        }
        part_[static_cast<size_t>(u)] = to;
        --current_size_[static_cast<size_t>(from)];
        ++current_size_[static_cast<size_t>(to)];
        // Add new bad contributions.
        for (int eid : incident[static_cast<size_t>(u)]) {
            if (!is_bad_pair(eid)) continue;
            const PairEdge &e = edges[static_cast<size_t>(eid)];
            if (!edge_bad[static_cast<size_t>(eid)]) {
                edge_bad[static_cast<size_t>(eid)] = 1;
                ++incident_bad[static_cast<size_t>(e.u)];
                ++incident_bad[static_cast<size_t>(e.v)];
                ++bad_count;
    ...
    cout << "Strict topology min-conflicts repair started: pair_edges=" << edges.size()
         << ", initial_pair_violations=" << bad_count
         << ", time_limit=" << time_limit_seconds << "s" << endl;

    while (bad_count > 0 && Clock::now() < deadline) {
        if (bad_edges.empty() || stale_picks > 200000 || steps % 500000 == 0) {
            int rebuilt = rebuild_bad_state();
            bad_count = rebuilt;
            stale_picks = 0;
            if (bad_count < best_bad) {
                best_bad = bad_count;
                best_part = part_;
                cout << "Strict repair progress: steps=" << steps
                     << ", pair_violations=" << bad_count
                     << ", net_violations=" << count_topology_violations(graph) << endl;
            }
            if (bad_count == 0) break;
        }

        std::uniform_int_distribution<int> pick_edge(0, static_cast<int>(bad_edges.size()) - 1);
        int eid = bad_edges[static_cast<size_t>(pick_edge(rng))];
        if (eid < 0 || eid >= static_cast<int>(edges.size()) || !edge_bad[static_cast<size_t>(eid)]) {
            ++stale_picks;
            ++steps;
            continue;
        }
        ...
        int u = e.u;
        int v = e.v;
        bool u_movable = !is_fixed(u) && current_size_[static_cast<size_t>(part_[static_cast<size_t>(u)])] > min_part_size_;
        bool v_movable = !is_fixed(v) && current_size_[static_cast<size_t>(part_[static_cast<size_t>(v)])] > min_part_size_;
        ...
        int chosen = -1;
        int chosen_to = -1;
        int current_conf = INT_MAX;
        int best_conf = INT_MAX;
        if (u_movable) {
            int to = choose_color(u, false);
            int conf = count_bad_if_color(u, to);
            int cur = incident_bad[static_cast<size_t>(u)];
            if (conf < best_conf || (conf == best_conf && cur > current_conf)) {
                chosen = u; chosen_to = to; best_conf = conf; current_conf = cur;
            }
        }
        if (v_movable) {
            int to = choose_color(v, false);
            int conf = count_bad_if_color(v, to);
            int cur = incident_bad[static_cast<size_t>(v)];
            if (conf < best_conf || (conf == best_conf && cur > current_conf)) {
                chosen = v; chosen_to = to; best_conf = conf; current_conf = cur;
            }
        }

        if (chosen >= 0 && chosen_to != part_[static_cast<size_t>(chosen)] && best_conf <= current_conf) {
            apply_move(chosen, chosen_to);
        } else {
            // Escape plateau: every few steps force one endpoint of a violated edge to change to
            // its best alternative, even if the local conflict count is unchanged.  This is the
            // classic min-conflicts restart/kick mechanism and is essential for strict non-hop.
            int endpoint = -1;
            if (u_movable && v_movable) {
                endpoint = (incident_bad[static_cast<size_t>(u)] >= incident_bad[static_cast<size_t>(v)]) ? u : v;
            } else {
                endpoint = u_movable ? u : v;
            }
            int to = choose_color(endpoint, true);
            if (to != part_[static_cast<size_t>(endpoint)]) apply_move(endpoint, to);
        }
    ...
    if (best_bad < bad_count) {
        part_ = best_part;
        std::fill(current_size_.begin(), current_size_.end(), 0);
        for (int p : part_) {
            if (p >= 0 && p < K_) ++current_size_[static_cast<size_t>(p)];
        }
        bad_count = rebuild_bad_state();
    }

    rebuild_net_counts(graph);
    cout << "Strict topology min-conflicts repair finished: moves=" << total_moves
         << ", remaining_pair_violations=" << bad_count
         << ", remaining_net_violations=" << count_topology_violations(graph) << endl;
    return total_moves;
}
...
void Solution::my_partition_algorithm(Graph &graph, int K, double r, vector<int> &part_result) {
    ...
    if (const char *env = std::getenv("VLSI_STRICT_SECONDS")) {
        strict_repair_seconds = std::max(0, std::atoi(env));
    }
    if (std::getenv("VLSI_STRICT_TOPO") != nullptr && strict_repair_seconds == 0) {
        strict_repair_seconds = 900;
    }
    ...
    for (int trial = 0; trial < num_restarts; ++trial) {
        ...
        if (use_fast_topology_mode) {
            ...
            if (strict_repair_seconds > 0) {
                strict_topology_min_conflicts(graph, strict_repair_seconds);
                rebuild_net_counts(graph);
            }
        } else {
            int pass = 0;
